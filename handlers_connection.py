"""Connection lifecycle for Fishbowl Inventory Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from fishbowl_client import FishbowlInventoryClient
...
@chat.function(
    "connect_fishbowl",
    "Connect Fishbowl Inventory account via credentials.",
    action_type="write",
    chain_callable=True,
    event="fishbowl-inventory-connector.connect_fishbowl",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_fishbowl(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Fishbowl Inventory Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Fishbowl Inventory Account",
...
@chat.function(
    "list_connections",
    "List connected Fishbowl Inventory accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_fishbowl",
    "Disconnect Fishbowl Inventory account.",
    action_type="write",
    chain_callable=True,
    event="fishbowl-inventory-connector.disconnect_fishbowl",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_fishbowl(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
