"""Resource handlers for Fishbowl Inventory Connector."""
...
@chat.function("list_shipments", "List shipments in Fishbowl Inventory.", action_type="read", chain_callable=True, event="fishbowl-inventory-connector.list_shipments", effects=["read:shipments"], data_model=ShipmentList)
...
@chat.function("get_shipment", "Get details of one Shipment in Fishbowl Inventory.", action_type="read", chain_callable=True, event="fishbowl-inventory-connector.get_shipment", effects=["read:shipment"], data_model=ShipmentRecord)
...
@chat.function("audit_shipment_health", "Audit health of Fishbowl Inventory shipments and connectivity.", action_type="read", chain_callable=True, event="fishbowl-inventory-connector.audit_shipment_health", effects=["read:audit"], data_model=AuditHealthReport)
async def audit_shipment_health(ctx, params: ConnectionIdParams) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.success({
            ...
            "summary": f"Fishbowl Inventory healthy. Sampled {len(items)} shipments."
        }, summary=f"Fishbowl Inventory health check passed with {len(items)} shipments.")
    except Exception as e:
        return ActionResult.error(f"Error auditing Fishbowl Inventory health: {e}")
